...
import { devices, type Employee, type Device } from "@/lib/mock-data";
...
const DEFAULT_EMPLOYEE: Employee = {
  ...
  status: "active",
  faceEnrolled: false,
  photo: "https://api.dicebear.com/7.x/notionists/svg?seed=Employee",
};
...
function KioskPage() {
  ...
  const [match, setMatch] = React.useState<Employee>(DEFAULT_EMPLOYEE);
    ...
    try {
      ...
      if (res.matched && res.employee) {
        setMatch({
          ...
          status: "active",
          faceEnrolled: true,
          photo: res.employee.photo || `https://api.dicebear.com/7.x/notionists/svg?seed=${encodeURIComponent(res.employee.name || "User")}`,
        });
        setConfidence(res.confidence || 0.94);
        setPhase("success");
      } else {
        setScanMessage(res.message || "Face not recognized — please enroll your face in Admin -> Employees first!");
        setPhase("failed");
      }
    } catch (err: any) {
  ...
  return (
    <div className="relative flex h-screen w-screen flex-col overflow-hidden bg-slate-950 text-slate-50">
          ...
          {phase === "success" && (
            <div className="flex flex-col items-center gap-6 animate-in fade-in zoom-in-95 duration-300">
              <div className="relative">
                <img
                  src={match?.photo || DEFAULT_EMPLOYEE.photo}
                  alt=""
              ...
              <h1 className="text-5xl font-semibold tracking-tight">{match?.name || "Employee"}</h1>
      ...
      <footer className="relative z-10 flex items-center justify-between px-8 py-5 text-sm text-slate-400">
        <span>
          {device?.name || "Front Desk Kiosk"} · {device?.location || "HQ · Lobby"}
        </span>
